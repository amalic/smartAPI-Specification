...
<a name="infoTermsOfService"></a>termsOfService | `string` | **REQUIRED**. A URL to the Terms of Service for the API. MUST be in the format of a URL.
...
Field Name | Type | Description
---|:---:|---
<a name="serverLocation"></a>x-location | `string` | Location, city and country of the server hosting the API.
...
```
A parameter with the smartAPI extension:
```yaml
name: username
in: path
description: username to fetch
...
<a name="responseSchema"></a> | `URI` | Specify the json schema using a Reference Object, or use "x-uri" as an additional property in the schema field to point to an external location for an XML, RDF, etc schema.
...
#### <a name="responseValueTypeObject"></a>Response Value Type Object [smartAPI extension]
 ...
 that should provide URI values to the type of object, and an optional `x-path` to specify to location in the response for that valueType.
...
Field Name | Type | Description
---|:---:|---
<a name="tagName"></a>x-id | `URI` | The name of the tag. Recommend that you use URI to specify the concept.
